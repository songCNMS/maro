...
LOG_SIG_MAX = 2
LOG_SIG_MIN = -20
TRAINING_EPISODES_PER_EVAL_EPISODE = 20
EVAL_EPISODE = 20
EPSILON = 1e-6
...
class SAC(Base_Agent):
    ...
    def eval(self):
        _state = self.eval_environment.reset()
        _done = False
        while not _done:
            _action = self.pick_action(True, state=_state)
            _state, _reward, _done, _ = self.eval_environment.step(_action)
        return self.eval_environment.print_eval_result(self.eval_environment.metrics)

    def step(self):
        def experience_from_horizon(transitions):
            state = transitions[0][0]
            action = transitions[0][1]
            mask = transitions[0][4]
            reward = 0
            for ts in transitions:
                reward += self.hyperparameters["discount_rate"] * ts[2]
            next_state = transitions[-1][3]
            return (state, action, reward, next_state, mask)
        
        print('step', self.episode_number + 1)
        ...
        while not self.done:
            ...
            transition_hist.append((self.state, self.action, self.reward, self.next_state, mask))
            if len(transition_hist) > self.config.env_horizon:
                transition_hist = transition_hist[-self.config.env_horizon:]
            if not eval_ep and len(transition_hist) > 1: self.save_experience(experience=experience_from_horizon(transition_hist))
    ...
    def pick_action(self, eval_ep, state=None):
        ...
        if eval_ep: action = self.actor_pick_action(state=state, eval=True)
            ...
            while True:
                ...
                print('random: ', action)
                if self.environment.action_mask[action] >= 1.0:
                    break
    ...
    def print_summary_of_latest_evaluation_episode(self):
        ...
        rl_res, greedy_res, greedy2_res, greedy_wtw_res = {}, {}, {}, {}
        for i in range(EVAL_EPISODE):
            print(f"------------------RL {i}-------------------------")
            _rl_res = self.eval()
            # print(f"--------------Greedy {i}-------------------------")
            # _greedy_res = self.eval_environment.get_greedy_metrics_env()
            print(f"--------------Greedy2 {i}-------------------------")
            _greedy2_res = self.eval_environment.get_greedy2_metrics_env()
            # print(f"-------------Greedy WTW {i}-----------------------")
            # _greedy_res_wtw = self.eval_environment.get_greedy_with_time_window_metrics_env()
            for key in _rl_res.keys():
                rl_res[key] = rl_res.get(key, 0) + _rl_res[key]
                # greedy_res[key] = greedy_res.get(key, 0) + _greedy_res[key]
                greedy2_res[key] = greedy2_res.get(key, 0) + _greedy2_res[key]
                # greedy_wtw_res[key] = greedy_wtw_res.get(key, 0) + _greedy_res_wtw[key]
        print("***************RL********************")
        print(rl_res)
        # print("*************Greedy******************")
        # print(greedy_res)

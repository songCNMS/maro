...
class SAC_Discrete(SAC):
    ...
    def produce_action_and_action_info(self, state_batch):
        """Given the state, produces an action, the probability of the action, the log probability of the action, and
        the argmax action"""
        state = state_batch[:, :-self.action_space_size]
        action_mask = state_batch[:, -self.action_space_size:]
        action_probabilities = self.actor_local(state) * action_mask
        action_probabilities = torch.clamp(action_probabilities, min=0.0)
    ...
    def calculate_critic_losses(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch):
        ...
        with torch.no_grad():
            ...
            qf1_next_target = self.critic_target(next_state_batch[:, :-self.action_space_size])
            qf2_next_target = self.critic_target_2(next_state_batch[:, :-self.action_space_size])
            min_qf_next_target = action_probabilities * (torch.min(qf1_next_target, qf2_next_target) - self.alpha * log_action_probabilities)
            min_qf_next_target = min_qf_next_target.sum(dim=1).unsqueeze(-1)
            next_q_value = reward_batch + (1.0 - mask_batch) * self.hyperparameters["discount_rate"] * (min_qf_next_target)

        qf1 = self.critic_local(state_batch[:, :-self.action_space_size]).gather(1, action_batch.long())
        qf2 = self.critic_local_2(state_batch[:, :-self.action_space_size]).gather(1, action_batch.long())
    ...
    def calculate_actor_loss(self, state_batch):
        ...
        qf1_pi = self.critic_local(state_batch[:, :-self.action_space_size])
        qf2_pi = self.critic_local_2(state_batch[:, :-self.action_space_size])
